...
def process_event_history(log: dict[str, list[dict]],
    ...
    for ev in log['events']:
        if ev['type'] == 'call':
            caller_number = ev['src_number']
            receiver_number = ev['dst_number']
            t = datetime.datetime.strptime(ev['time'], "%Y-%m-%d %H:%M:%S")
            call_duration = int(ev['duration'])
            caller_location = ev['src_loc']
            receiver_location = ev['dst_loc']

            call = Call(caller_number, receiver_number, t, call_duration,
                        caller_location, receiver_location)

            if is_first_event:
                new_month(customer_list, t.month, t.year)
            ...
            caller_customer = find_customer_by_number(caller_number,
                                                      customer_list)
            receiver_customer = find_customer_by_number(receiver_number,
                                                        customer_list)

            if caller_customer and receiver_customer:
                caller_customer.make_call(call)
                receiver_customer.receive_call(call)
