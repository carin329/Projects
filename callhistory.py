...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        month_and_year = (call.time.month, call.time.year)
        if month_and_year not in self.outgoing_calls:
            self.outgoing_calls[month_and_year] = []
        self.outgoing_calls[month_and_year].append(call)

    def register_incoming_call(self, call: Call) -> None:
        """ Register a Call <call> into this incoming call history
        """
        month_and_year = (call.time.month, call.time.year)
        if month_and_year not in self.incoming_calls:
            self.incoming_calls[month_and_year] = []
        self.incoming_calls[month_and_year].append(call)
    # ----------------------------------------------------------
